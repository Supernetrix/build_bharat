...
export function LoginForm() {
    ...
    const handleSendOTP = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.success) {
                setStep("otp")
                // Check if this is a new user based on response
                setIsNewUser(response.message?.includes("new") || false)
            }
        } catch (error) {
    ...
    const handleVerifyOTP = async (e: React.FormEvent) => {
        ...
        if (isNewUser && (!username.trim() || !fullName.trim())) return
        ...
        try {
            ...
            if (isNewUser) {
                // Use signup endpoint for new users
                response = await authService.signup(phoneNumber, otp, username, fullName)
            } else {
                // Use login endpoint for existing users
                response = await authService.login(phoneNumber, otp)
            }
            ...
            {step === "phone" ? (
                ...
                <form onSubmit={handleVerifyOTP} className="space-y-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button
                            type="submit"
                            disabled={isLoading || otp.length !== 6 || (isNewUser && (!username.trim() || !fullName.trim()))}
